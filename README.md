...
- **FROST threshold signatures via Nostr** — FROST (Flexible Round-Optimized Schnorr Threshold) enables t-of-n multisig on Alephium with no on-chain footprint. The distributed key generation ceremony and two-round signing protocol are coordinated over NIP-44 encrypted DMs on Nostr relays. The output is a standard BIP-340 Schnorr signature — Alephium verifies it like any single-signer transaction against the aggregated group public key (mapped to a P2SH address). Any t-of-n subset can sign, the group key doubles as a shared Nostr identity, and participants never reveal their individual secret shares. Also supports MuSig2 (n-of-n) for simpler cases
